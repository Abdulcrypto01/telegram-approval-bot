...
async def handle_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cur.execute("INSERT INTO submissions (user_id, link, status) VALUES (?, ?, ?)",
    ...
    await update.message.reply_text("⏳ An karɓa, ana jira approve.")
...
async def mypoints(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row = cur.fetchone()
    await update.message.reply_text(f"⭐ Points: {row[0] if row else 0}")
